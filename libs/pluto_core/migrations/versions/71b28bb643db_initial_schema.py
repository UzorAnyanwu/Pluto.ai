...
def upgrade() -> None:
    # Not autogenerated — Alembic's reflection-based diff doesn't know about extensions. Required
    # before `knowledge_chunks.embedding` (a `vector` column, created further down) can exist.
    # Making this migration create the extension itself (rather than relying on a bootstrap
    # script run once per environment, as local dev and docker-compose currently do) means this
    # migration is self-contained on any fresh Postgres 16+ instance — including a managed
    # provider's default database, which is exactly the case that motivated this fix (see
    # PROJECT_STATUS.md's Render deployment notes).
...
def downgrade() -> None:
    ...
    # Also not autogenerated: dropping a table does not drop the Postgres ENUM types its columns
    # used (SQLAlchemy/Alembic has no "is this type still referenced elsewhere" tracking during a
    # bare drop_table). Without this, `downgrade()` leaves every enum type behind, and the next
    # `upgrade()` fails with "type already exists" the moment it tries to recreate one — caught by
    # actually running the up/down/up cycle CI performs, not just `upgrade` alone.
